...
public class SleepingSheepFurModel
extends SheepFurModel<Sheep> {
    ...
    public SleepingSheepFurModel(ModelPart root) {
        super(root);
        this.modelRoot = root;
    ...
    public void setupAnim(Sheep entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (sleeping || resting) {
            SleepPose.apply(this.parts, 13.5f, SleepPose.headY(20.0f, entity.isBaby(), 10.0f), -8.0f, 8.0f, this.defaultRightFrontX, this.defaultLeftFrontX, this.defaultRightHindX, this.defaultLeftHindX, 0.0f, 1.5f, resting, this.defaultBodyY, this.defaultHeadY);
            if (sleeping) {
                head.yRot = 0.0f;
            }
            body.yRot = 0.0f;
