...
public class SleepingCowModel
extends CowModel<Cow> {
    ...
    public SleepingCowModel(ModelPart root) {
        super(root);
        this.modelRoot = root;
    ...
    public void setupAnim(Cow entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if ((sleeping || resting) && SleepPose.lieDownModelsEnabled()) {
            ...
            if (HeadDip.active((LivingEntity)entity, 0.0f)) {
                head.y = this.defaultHeadY + HeadDip.positionScale((LivingEntity)entity, 0.0f) * 4.0f;
