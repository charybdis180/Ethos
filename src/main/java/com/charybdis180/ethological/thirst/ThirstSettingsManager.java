...
extends SimpleJsonResourceReloadListener {
    ...
    private static final Map<EntityType<?>, Optional<SpeciesThirstSettings>> RESOLVED = new ConcurrentHashMap<EntityType<?>, Optional<SpeciesThirstSettings>>();
    ...
    public static Optional<SpeciesThirstSettings> get(EntityType<?> type) {
        Optional<SpeciesThirstSettings> cached = RESOLVED.get(type);
        if (cached != null) {
            return cached;
        }
        SpeciesThirstSettings datapack = settingsByType.get(type);
        Optional<SpeciesThirstSettings> resolved = datapack == null ? Optional.empty() : Optional.of(EthologicalConfig.resolveThirst(type, datapack));
