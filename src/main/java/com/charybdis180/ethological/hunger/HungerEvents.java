...
public final class HungerEvents {
    ...
    public static void onAddReloadListeners(AddReloadListenerEvent event) {
        event.addListener((PreparableReloadListener)new HungerSettingsManager());
    }
    ...
    public static void onEntityJoinLevel(EntityJoinLevelEvent event) {
        ...
        if (!animal.hasData(HungerAttachments.HUNGER_DATA)) {
        ...
        animal.goalSelector.addGoal(5, (Goal)new EatFoodGoal(animal, 1.0, 16, 3));
    ...
    public static void onEntityInteract(PlayerInteractEvent.EntityInteract event) {
        ...
        Animal animal = (Animal)entity;
        Optional<SpeciesHungerSettings> settingsOpt = HungerSettingsManager.get(animal.getType());
        if (settingsOpt.isEmpty() || !animal.hasData(HungerAttachments.HUNGER_DATA)) {
            return;
        }
        SpeciesHungerSettings settings = settingsOpt.get();
    ...
    private static int effectiveDepletionInterval(Animal animal, SpeciesHungerSettings settings) {
        Sheep sheep;
        float interval = (float)settings.depletionIntervalTicks() * HungerEvents.individualPace(animal);
    ...
    public static void onBabySpawn(BabyEntitySpawnEvent event) {
        ...
        baby.setData(HerdAttachments.MOTHER, MotherData.create(mother.getUUID(), now, followTicks));
        if (mother.hasData(HerdAttachments.HERD_DATA) && mother.level() instanceof ServerLevel) {
            HerdManager.Herd herd = HerdManager.getOrCreate(((HerdData)mother.getData(HerdAttachments.HERD_DATA)).herdId(), mother.getUUID());
            herd.members.add(baby.getUUID());
            baby.setData(HerdAttachments.HERD_DATA,new HerdData(herd.id, false));
        }
    }
