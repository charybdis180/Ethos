...
extends SimpleJsonResourceReloadListener {
    ...
    private static final Map<EntityType<?>, Optional<SpeciesSleepSettings>> RESOLVED = new ConcurrentHashMap<EntityType<?>, Optional<SpeciesSleepSettings>>();
    ...
    public static Optional<SpeciesSleepSettings> get(EntityType<?> type) {
        Optional<SpeciesSleepSettings> cached = RESOLVED.get(type);
        if (cached != null) {
            return cached;
        }
        SpeciesSleepSettings datapack = settingsByType.get(type);
        Optional<SpeciesSleepSettings> resolved = datapack == null ? Optional.empty() : Optional.of(EthologicalConfig.resolveSleep(type, datapack));
